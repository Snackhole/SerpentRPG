...
class InstallerWindow(QWidget):
    def __init__(self):
        ...
        # Variables
        self.WindowIcon = QIcon(self.ResourcePath("Source/Assets/SerpentRPG Icon.png"))
        self.ScriptName = os.path.splitext(os.path.basename(__file__))[0]
        self.SerpentRPGScriptName = self.ScriptName[:-10]
        self.ProgramZip = self.ResourcePath("Executables/Final/" + self.SerpentRPGScriptName + ".zip")
    ...
    def Install(self):
        ...
        if self.DisplayMessageBox("WARNING:  All files currently in the chosen location will be deleted and replaced with SerpentRPG's files.  Proceed?", Buttons=(QMessageBox.Yes | QMessageBox.No),
        ...
        try:
            for File in os.listdir(InstallLocation):
                ...
                if os.path.isfile(FilePath) and File != "LastOpenedDirectory.cfg":
                    os.unlink(FilePath)
            ...
            if self.DesktopShortcutCheckBox.isChecked():
                ShortcutTarget = os.path.join(InstallLocation, "SerpentRPG.exe")
                Shell = Dispatch("WScript.Shell")
                Shortcut = Shell.CreateShortCut(os.path.join(winshell.desktop(), "SerpentRPG.lnk"))
        ...
        self.DisplayMessageBox("SerpentRPG has been installed to the chosen location.")
        self.close()
